...
- [x] **Desktop notifications** — added `tauri-plugin-notification`, a `Settings → General` toggle, and `lib/notify.ts`. Fetch/pull/push notify on completion only if they took over 4s (skips near-instant ones). PR rows get a bell toggle to "watch" a PR; a background 60s poller (`usePRStore.pollWatchedChecks`, wired in `App.tsx`) notifies when a watched PR's CI status flips to passing/failing.
- [x] **Multi-repo batch actions** — "Batch" button in the sidebar header (`BatchSyncPanel.tsx`) runs Fetch All / Pull All across every sidebar repo (4 at a time via a small worker pool in `useBatchSyncStore`), with a combined progress panel (per-repo pending/running/done/error) instead of repo-by-repo.
...
- [x] **Repo workspaces/groups** — new `workspaces.rs` backend + `WorkspacePicker`/`WorkspaceDialog`: named, user-defined sets of repos (independent of the auto-derived owner grouping and sidebar sections) saved to `workspaces.json`. Picking one in the sidebar filters the repo list to just that set; "Batch" then fetches/pulls exactly that filtered set together.
- [x] **Auto-update** (mechanism wired, needs real release infra to activate) — added `tauri-plugin-updater`/`tauri-plugin-process` and a `Settings → Advanced → Updates` "Check for Updates" flow (check → show version/notes → download & install → relaunch). Since this repo has no signed-release pipeline yet, `tauri.conf.json` intentionally has no `plugins.updater.endpoints`/`pubkey` set — the UI detects that and shows "not configured" rather than erroring oddly. To go live: run `tauri signer generate`, put the pubkey + a versions.json endpoint (e.g. hosted off GitHub Releases) into `tauri.conf.json`, and sign each release build — no code changes needed after that.
- [x] **Git LFS awareness** — new `lfs.rs` backend (`.gitattributes` detection + `git check-attr filter` for per-file tracking + LFS pointer-file size parsing); `FileList.tsx` shows an "LFS · size" badge on tracked files, and a toolbar `LfsPanel` (shown only when the repo uses LFS) runs `git lfs pull`/`push` through the same streaming/progress-toast path as fetch/pull/push instead of silently stalling.
- [x] **Reflog / undo UI** — new `reflog.rs` backend (`get_reflog`/`reflog_restore`) + a toolbar `ReflogPanel` dialog listing every place HEAD has pointed, with a 1-click "Restore" (hard reset, gated behind an explicit "Confirm reset" second click since it's destructive).
