...
- [ ] Repo > Toolbar (top) > Next to the open on git provider button: "Open in <editor>" (editor icon as the button, just like the open on git provider button)
- [ ] when creating a new branch, i can see the last commit that was made (to the branch i am coming from) below the commit button and the corresponding undo button. this happens only while the new branch is local-only. as soon as i publish it, the last commit is no longer visible. this last commit was already pushed and should not have been visible in the first place. fix this.
