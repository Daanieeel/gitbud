...
const ENTRY_FILES = new Set(["index.ts", "environment.ts", "manifest.ts", "proxy.ts", "env.js"]);
...
function stripCommentsAndStrings(source: string): string {
  const out: string[] = [];
  const stack: ScanFrame[] = [{ kind: "code", interpolation: false, braceDepth: 0 }];
...
async function main(): Promise<number> {
  ...
  for (const source of sources.values()) {
    for (const token of stripCommentsAndStrings(source).matchAll(/[A-Za-z_$][\w$]*/g)) {
      const name = token[0];
  ...
  if (dead.length === 0) {
    process.stdout.write(`no unreferenced exports (${exported.length} checked)\n`);
    return 0;
  }
  process.stdout.write(`§14.14 unreferenced export — ${dead.length} violation(s)\n`);
  for (const symbol of dead) {
    process.stdout.write(`  ${symbol.file}:${symbol.line}  ${symbol.name}\n`);
  }
  process.stdout.write("  fix: delete it, or stop exporting it if it is module-internal\n");
  return 1;
}
